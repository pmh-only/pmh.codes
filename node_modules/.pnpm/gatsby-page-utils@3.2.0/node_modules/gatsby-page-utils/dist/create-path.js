...
function createPath(filePath, withTrailingSlash = false, usePathBase = false) {
  const {
    ...
    name,
    base
  } = (0, _path.parse)(filePath); // When a collection route also has client-only routes (e.g. {Product.name}/[...sku])
  ...
  const postfix = withTrailingSlash ? `/` : ``; // Convert slashes since the Regex operates on forward slashes
