...
function warnForDeprecation() {
  ...
  console.warn(`[gatsby-transformer-sharp] The "fixed" and "fluid" resolvers are now deprecated. Switch to "gatsby-plugin-image" for better performance and a simpler API. See https://gatsby.dev/migrate-images to learn how.`);
}
        ...
        tracedSVG: {
          ...
          resolve: parent => {
            if (!didShowTraceSVGRemovalWarningFixed) {
              console.warn(`"tracedSVG" placeholder field is no longer supported (used in ImageSharp.fixed processing), falling back to "base64". See https://gatsby.dev/tracesvg-removal/`);
        ...
        tracedSVG: {
          ...
          resolve: parent => {
            if (!didShowTraceSVGRemovalWarningFluid) {
              console.warn(`"tracedSVG" placeholder field is no longer supported (used in ImageSharp.fluid processing), falling back to "base64". See https://gatsby.dev/tracesvg-removal/`);
...
const imageNodeType = ({
  ...
  return {
    type: hasFeature(`graphql-typegen`) ? `GatsbyImageData!` : new GraphQLNonNull(GraphQLJSON),
    args: {
      layout: {
    ...
    resolve: async (image, fieldArgs, context) => {
      ...
      if ((fieldArgs === null || fieldArgs === void 0 ? void 0 : fieldArgs.placeholder) === `tracedSVG`) {
        if (!didShowTraceSVGRemovalWarningGatsbyImageData) {
          console.warn(`"TRACED_SVG" placeholder argument value is no longer supported (used in ImageSharp.gatsbyImageData processing), falling back to "DOMINANT_COLOR". See https://gatsby.dev/tracesvg-removal/`);
          ...
          tracedSVG: {
            ...
            resolve: async parent => {
              if (!didShowTraceSVGRemovalWarningResize) {
                console.warn(`"tracedSVG" placeholder field is no longer supported (used in ImageSharp.resize processing), falling back to "base64". See https://gatsby.dev/tracesvg-removal/`);
