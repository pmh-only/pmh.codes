...
const buildPartialHydrationRenderer = async (program, stage, parentSpan) => {
  ...
  config.cache = false;
  config.output.path = path.join(program.directory, `.cache`, `partial-hydration`); // require.resolve might fail the build if the package is not installed
...
const renderPartialHydrationQueue = async (workerPool, activity, pages, program) => {
  ...
  const {
    assetPrefix,
    pathPrefix
  } = config; // Let the error bubble up
...
async function buildSlices({
  ...
  const state = _redux.store.getState(); // for now we always render everything, everytime
