...
async function writePageData(publicDir, pageData, slicesUsedByTemplates, slices) {
  ...
  const body = (0, _pageDataHelpers.constructPageDataString)(pageData, result, slicesUsedByTemplates, slices); // transform asset size to kB (from bytes) to fit 64 bit to numbers
...
async function flush(parentSpan) {
  ...
  const flushQueue = (0, _fastq.default)(async (task, cb) => {
    if (task.type === `page`) {
      ...
      let shouldClearPendingWrite = true; // It's a gloomy day in Bombay, let me tell you a short story...
